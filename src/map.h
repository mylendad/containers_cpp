...
namespace s21
{
  ...
  template <typename Key, typename T, bool AllowDuplicate = false>
  ...
  public:
    ...
    // map Member type
    using iterator = typename Tree<std::pair<const Key, T>, AllowDuplicate>::TreeIterator;
    using typename Tree<const Key, T, bool AllowDuplicate>::size_type;
    using key_type = Key;
